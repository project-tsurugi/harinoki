...
```
なお、鍵長（2048）は、4096でも良い。RSA暗号化で1ブロックに暗号化できる最大のデータサイズは、パディング方式によって異なります。例えば、OAEPパディングを用いる場合、最文サイズは「鍵長（バイト数） - 2×ハッシュ長（バイト数） - 2」となります（例：2048ビット鍵＋SHA-1の場合は214バイト）。単純に「鍵長（ビット数）÷8」よりも小さくなる点に注意してください。
