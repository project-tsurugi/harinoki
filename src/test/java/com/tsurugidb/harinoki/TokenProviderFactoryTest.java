...
class TokenProviderFactoryTest {
    ...
    static class Mock extends TokenProviderFactory {
        ...
        String privateKey(Path pemFile) throws IOException {
            if (pemFile.endsWith(DEFAULT_PRIVATE_KEY)) {
                return TestConstants.privateKey();
            }
    ...
    void simple() throws Exception {
        ...
        assertArrayEquals(
                Algorithm.RSA256(TokenProviderFactory.createPublicKey(TestConstants.publicKey()), TokenProviderFactory.createPrivateKey(TestConstants.privateKey())).sign(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
